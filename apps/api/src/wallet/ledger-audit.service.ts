...
export class LedgerAuditService {
     ...
     */
    async runAudit(): Promise<{ id: string; status: string; discrepancyCount: number }> {
        ...
        try {
            ...
            const auditLog = await this.prisma.ledgerAuditLog.create({
                data: {
                    ...
                    status,
                    durationMs,
                    details: discrepancies.length > 0 ? JSON.parse(JSON.stringify(discrepancies)) : undefined,
                },
            });
