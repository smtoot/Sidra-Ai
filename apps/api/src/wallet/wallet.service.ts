...
export class WalletService {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updatedTx = await tx.transaction.update({
        ...
        data: {
          ...
          paidAt:
            dto.status === TransactionStatus.PAID ? new Date() : undefined,
        },
      });
        ...
        if (dto.status === TransactionStatus.PAID) {
          ...
          if (!dto.referenceId) {
            throw new BadRequestException(
              'Receipt Number (Reference ID) is mandatory for confirming payment',
            );
          }
      ...
      // Logic for Deposit:
        ...
        if (
          ...
          if (!updatedWallet) {
            this.logger.error(
              `Wallet not found for user ${userId} after deposit approval`,
            );
            return updatedTx;
          }
          ...
          this.logger.log(
            `Deposit approved for user ${userId}. Found ${pendingBookings.length} pending bookings.`,
          );
          ...
          for (const booking of pendingBookings) {
            ...
            if (currentAvailableBalance >= price) {
              this.logger.log(
                `Auto-paying booking ${booking.id} (price: ${price} SDG)`,
              );
              ...
              try {
                ...
                this.logger.error(
                  `Failed to auto-pay booking ${booking.id}: ${error.message}`,
                );
                // Continue to next booking - don't fail the deposit approval
              }
            } else {
              this.logger.log(
                `Insufficient balance for booking ${booking.id}. Required: ${price}, Available: ${currentAvailableBalance}`,
              );
  ...
  async lockFundsForBooking(
    ...
    const operation = async (transaction: Prisma.TransactionClient) => {
      ...
      if (updateResult.count === 0) {
        throw new BadRequestException(
          'Insufficient balance (concurrent request detected)',
        );
      }
      ...
      this.logger.log(
        `💰 LOCK | bookingId=${bookingId} | txId=${txRecord.id} | amount=${normalizedAmount} | walletId=${wallet.id}`,
      );
    };
  ...
  async releaseFundsOnCompletion(
    ...
    if (Number(parentWallet.pendingBalance) < amount) {
      throw new BadRequestException(
        `Insufficient pending balance. Available: ${parentWallet.pendingBalance}, Required: ${amount}`,
      );
    }
    ...
    const teacherEarnings = normalizeMoney(
      normalizedAmount * (1 - commissionRate),
    );
  ...
  async settleCancellation(
    ...
    if (Number(parentWallet.pendingBalance) < totalLockedAmount) {
      throw new BadRequestException(
        `Insufficient pending balance. Available: ${parentWallet.pendingBalance}, Required: ${totalLockedAmount}`,
      );
    }
  ...
  async upsertBankInfo(userId: string, dto: UpsertBankInfoDto) {
    ...
    if (dto.iban && dto.iban.length < 15)
  ...
  async requestWithdrawal(userId: string, dto: WithdrawalRequestDto) {
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (activeWithdrawal) {
          throw new BadRequestException(
            'You have an active withdrawal request. Please wait for it to be processed.',
          );
        }
        ...
        if (updateResult.count === 0) {
          throw new BadRequestException(
            'Insufficient balance or concurrent transaction failure',
          );
        }
