...
export class AdminTeamService {
    ...
    async createAdminUser(
        ...
        if (!CREATABLE_ADMIN_ROLES.includes(data.role as any)) {
            throw new BadRequestException(`Invalid role: ${data.role}. Allowed roles: ${CREATABLE_ADMIN_ROLES.join(', ')}`);
        }
        ...
        const existingEmail = data.email ? await this.prisma.user.findUnique({
        ...
        if (data.permissionOverrides) {
            const errors = this.permissionService.validateOverrides(data.permissionOverrides);
            if (errors.length > 0) {
                throw new BadRequestException(`Invalid permission overrides: ${errors.join(', ')}`);
            }
        }
        ...
        const user = await this.prisma.user.create({
            data: {
                ...
                permissionOverrides: data.permissionOverrides ? JSON.parse(JSON.stringify(data.permissionOverrides)) : undefined,
    ...
    async updatePermissionOverrides(
        ...
        if (targetUser.role === 'ADMIN' && actorRole !== 'SUPER_ADMIN') {
            throw new ForbiddenException('Only SUPER_ADMIN can modify ADMIN permissions');
        }
     ...
     */
    async deactivateAdminUser(actorId: string, actorRole: string, targetUserId: string) {
        ...
        if (targetUser.role === 'ADMIN' && actorRole !== 'SUPER_ADMIN') {
            throw new ForbiddenException('Only SUPER_ADMIN can deactivate ADMIN users');
        }
