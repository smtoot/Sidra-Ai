...
export class NotificationService {
     ...
     */
    async createInAppNotification(params: CreateNotificationParams): Promise<boolean> {
        try {
    ...
    async notifyUser(params: NotifyUserParams): Promise<void> {
        ...
        if (params.email?.to) {
            ...
            this.logger.warn(`Email params provided but missing 'to' field for user ${params.userId}`);
        }
    }
    ...
    async notifySessionComplete(params: {
        ...
        await this.notifyUser({
            ...
            email: user?.email ? {
                ...
                payload: { teacherName, bookingUrl: `${process.env.FRONTEND_URL || ''}/bookings/${bookingId}` }
            } : undefined
        });
    }
    ...
    async notifyDisputeWindowReminder(params: {
        ...
        await this.notifyUser({
            ...
            metadata: { bookingId: params.bookingId, hoursRemaining: params.hoursRemaining },
            email: user?.email ? {
                to: user.email,
                subject: `Reminder: ${params.hoursRemaining} Hours to Review Session`,
                templateId: 'dispute-window-reminder',
                payload: { hoursRemaining: params.hoursRemaining, teacherName: params.teacherName }
            } : undefined
        });
    }
    ...
    async notifyTeacherPaymentReleased(params: {
        ...
        const releaseTypeArabic = params.releaseType === 'AUTO' ? 'تلقائي' : 'تأكيد الطالب';
        ...
        await this.notifyUser({
            ...
            link: '/teacher/wallet',
            metadata: { bookingId: params.bookingId, amount: params.amount, releaseType: params.releaseType },
            email: teacher.email ? {
                to: teacher.email,
                subject: 'Payment Released to Your Wallet',
                templateId: 'teacher-payment-released',
                payload: { amount: params.amount, releaseType: params.releaseType === 'AUTO' ? 'Automatic' : 'Student Confirmation' }
            } : undefined
        });
    }
}
