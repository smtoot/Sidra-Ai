...
describe('EmailOutboxWorker', () => {
    ...
    describe('retry cap and FAILED state', () => {
        it('should mark email as FAILED after 5 attempts', async () => {
            ...
            (prisma.emailOutbox.findMany as jest.Mock).mockResolvedValue([emailWith4Attempts]);
            (prisma.emailOutbox.updateMany as jest.Mock).mockResolvedValue({ count: 1 });

            // Mock the update to simulate the failure handling
            (prisma.emailOutbox.update as jest.Mock).mockResolvedValue({ id: 'email-1' });
