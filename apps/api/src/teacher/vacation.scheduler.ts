...
export class VacationScheduler {
    ...
    async handleVacationExpiry() {
        ...
        try {
            ...
            this.logger.log(`Found ${expiredTeachers.length} expired vacations to process`);
            ...
            this.logger.log(`Auto-returned ${updateResult.count} teachers from vacation`);
            ...
            for (const teacher of expiredTeachers) {
                try {
                    await this.notificationService.notifyUser({
                        ...
                        message: 'تم إيقاف وضع الإجازة تلقائياً. أنت الآن متاح للحجوزات الجديدة.',
                        type: 'SYSTEM_ALERT',
                        link: '/teacher/settings',
                        dedupeKey: `VACATION_ENDED:${teacher.id}:${now.toISOString().split('T')[0]}`
                    });
                } catch (error) {
                    this.logger.error(`Failed to notify teacher ${teacher.id} about vacation end`, error);
                }
            }
        } catch (error) {
