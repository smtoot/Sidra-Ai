...
export class PackageService {
    ...
    async createTier(data: CreatePackageTierDto) {
        ...
        if (Math.abs((data.recurringRatio + data.floatingRatio) - 1.0) > 0.001) {
            throw new BadRequestException('recurringRatio + floatingRatio must equal 1.0');
        }
    ...
    async updateTier(id: string, data: UpdatePackageTierDto) {
        ...
        return this.prisma.packageTier.update({
            ...
            data: {
                ...
                ...(data.recurringRatio !== undefined && { recurringRatio: new Decimal(data.recurringRatio) }),
                ...(data.floatingRatio !== undefined && { floatingRatio: new Decimal(data.floatingRatio) }),
    ...
    async purchasePackage(
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            try {
                ...
                const teacherName = teacher?.user?.phoneNumber || teacher?.user?.email || 'المعلم';
    ...
    async purchaseSmartPackage(data: PurchaseSmartPackDto) {
        ...
        if (tierSetting && !tierSetting.isEnabled) {
            throw new BadRequestException('This package tier is not available from this teacher');
        }
        ...
        if (!tier.isActive) {
            throw new BadRequestException('This package tier is not currently available');
        }
        ...
        if (payerWallet.balance.lessThan(totalPaid)) {
            throw new BadRequestException('Insufficient balance for Smart Pack purchase');
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            try {
                ...
                // Log error but don't fail the purchase
                console.error('Failed to send Smart Pack purchase notification:', error);
            }
    ...
    async checkRecurringAvailability(data: CheckRecurringAvailabilityDto): Promise<RecurringAvailabilityResponse> {
    ...
    async bookFloatingSession(packageId: string, userId: string, data: BookFloatingSessionDto) {
    ...
    async reschedulePackageSession(bookingId: string, userId: string, data: RescheduleSessionDto) {
        ...
        if (booking.status !== 'SCHEDULED') {
            throw new BadRequestException(`Cannot reschedule: booking is ${booking.status}`);
        }
    ...
    async updateTeacherDemoSettings(teacherId: string, data: UpdateTeacherDemoSettingsDto) {
    ...
    async updateTeacherTierSetting(teacherId: string, tierId: string, data: UpdateTeacherTierSettingDto) {
        // Verify tier exists
    ...
    async expireSmartPacks() {
        ...
        for (const pkg of expiredPackages) {
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                const refundAmount = normalizeMoney(pkg.discountedPricePerSession.toNumber() * unusedFloating);
