...
export class ConfigValidationService implements OnModuleInit {
    ...
    onModuleInit() {
        ...
        if (!this.configService.get('AWS_ACCESS_KEY_ID') || !this.configService.get('AWS_SECRET_ACCESS_KEY')) {
            this.logger.warn('⚠️  AWS credentials not configured - file upload will be disabled');
        } else {
        ...
        if (!this.configService.get('SENDGRID_API_KEY')) {
            this.logger.warn('⚠️  SENDGRID_API_KEY not configured - email will be logged only (phone-first: this is acceptable)');
        } else {
        ...
        if (errors.length > 0) {
            ...
            throw new Error(`Application startup failed: Missing critical environment variables`);
        }
