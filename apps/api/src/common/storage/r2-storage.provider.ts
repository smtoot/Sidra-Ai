...
export class R2StorageProvider implements StorageProvider {
    ...
    constructor(config: R2Config) {
        ...
        this.logger.log(`R2 Storage Provider initialized for bucket: ${this.bucketName}`);
    }
     ...
     */
    async save(buffer: Buffer, fileKey: string, contentType?: string): Promise<void> {
        try {
            ...
            this.logger.error(`Failed to save file to R2: ${error.message}`, error.stack);
    ...
    private async getStreamAsync(fileKey: string, passThrough: any): Promise<void> {
     ...
     * @param expiresIn - Expiration time in seconds (default: 1 hour)
     */
    async getUploadUrl(fileKey: string, contentType: string, expiresIn: number = 3600): Promise<string> {
        const command = new PutObjectCommand({
            Bucket: this.bucketName,
            Key: fileKey,
