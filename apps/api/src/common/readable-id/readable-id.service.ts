...
export class ReadableIdService {
    ...
    async generate(type: 'BOOKING' | 'TRANSACTION' | 'PACKAGE' | 'WALLET', date?: Date): Promise<string> {
        const prefixMap = {
            BOOKING: 'BK',
            TRANSACTION: 'TX',
            PACKAGE: 'PKG',
    ...
    private async generateSequentialId(type: any, yearMonth: string, prefix: string, padding: number): Promise<string> {
