...
export class SlugService {
    ...
    private readonly arabicToEnglish: Record<string, string> = {
        ...
        'ه': 'h', 'ة': 'a', 'و': 'w', 'ي': 'y', 'ى': 'a', 'ئ': 'e', 'ء': '',
        'ؤ': 'o', 'لا': 'la', 'ً': '', 'ٌ': '', 'ٍ': '', 'َ': '', 'ُ': '', 'ِ': '',
        'ّ': '', 'ْ': '',
    };
    ...
    validateSlug(slug: string): { valid: boolean; error?: string } {
        ...
        // Reserved slugs
        const reserved = ['admin', 'api', 'www', 'mail', 'support', 'help', 'search', 'teachers', 'login', 'register'];
     ...
     */
    async isSlugAvailable(slug: string, excludeTeacherId?: string): Promise<boolean> {
     ...
     */
    async generateUniqueSlug(displayName: string, excludeTeacherId?: string): Promise<string> {
    ...
    async setTeacherSlug(
        ...
        if (teacher.slugLockedAt) {
            throw new BadRequestException('Slug is already locked and cannot be changed');
        }
