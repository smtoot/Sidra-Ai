...
async function run() {
    ...
    // Initial deposit
    await walletService.deposit(user.id, { amount: 2000, referenceImage: 'http://test.com/ref.jpg' });
    // Auto-approve deposit to get funds
    const depositTx = await prisma.transaction.findFirst({
        where: { walletId: (await walletService.getBalance(user.id)).id, type: 'DEPOSIT' as any }
    });
    ...
    if (Number(walletAfterReq.balance) !== Number(initialWallet.balance) - 500) throw new Error('Balance not deducted!');
    if (Number(walletAfterReq.pendingBalance) !== Number(initialWallet.pendingBalance) + 500) throw new Error('Pending balance not incremented!');
    ...
    if (Number(walletAfterReject.balance) !== Number(initialWallet.balance)) throw new Error('Balance not refunded!');
    if (Number(walletAfterReject.pendingBalance) !== Number(initialWallet.pendingBalance)) throw new Error('Pending balance not cleared!');
    ...
    const tx2Approved = await prisma.transaction.findUnique({ where: { id: tx2.id } });
    if (!tx2Approved || tx2Approved.status !== 'APPROVED') throw new Error('Status not updated to APPROVED');
    ...
    if (Number(walletAfterPaid.balance) !== Number(initialWallet.balance) - 800) throw new Error('Balance should remain deducted');
    if (Number(walletAfterPaid.pendingBalance) !== 0) throw new Error('Pending balance should be burned (0)');

    // 6. Test 4: Concurrency
    console.log('\n--- TEST 4: Concurrency (Race Condition) ---');
    // Set balance to 2000. Try 5 requests of 300.
    await prisma.wallet.update({ where: { id: walletAfterPaid.id }, data: { balance: 2000, pendingBalance: 0 } });
    ...
    if (successes.length > 1) {
        throw new Error(`CRITICAL: Concurrency check failed! Allowed ${successes.length} overlapping withdrawals.`);
    } else {
