...
describe('AdminBookingControl Integration', () => {
  ...
  describe('Admin Reschedule', () => {
    it('should reschedule booking when teacher is available and no conflict exists', async () => {
      ...
      expect(mockTeacherService.isSlotAvailable).toHaveBeenCalledWith(
        'teacher-1',
        newStart,
      );
    ...
    it('should throw BadRequest if teacher is NOT available', async () => {
      ...
      await expect(
        bookingService.adminReschedule(bookingId, new Date()),
  ...
  describe('Admin Force Complete', () => {
    it('should allow ADMIN to confirm session even if not owner', async () => {
      ...
      const booking = {
        ...
        teacherProfile: {
          userId: 'teacher-1',
          user: { id: 'teacher-1', email: 't@t.com' },
        },
      ...
      // Call as ADMIN
      await bookingService.confirmSessionEarly(
        adminId,
        bookingId,
        undefined,
        'ADMIN',
    ...
    it('should BLOCK non-owner if role is not ADMIN', async () => {
      ...
      await expect(
        bookingService.confirmSessionEarly(intruderId, bookingId),
