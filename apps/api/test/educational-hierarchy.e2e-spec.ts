...
describe('Educational Hierarchy & Search (E2E Contract)', () => {
    ...
    beforeAll(async () => {
        ...
        const loginRes = await request(app.getHttpServer())
            .post('/auth/login')
            .send({ email: `test_teacher_${Date.now()}@sidra.com`, password: 'password123' });
    ...
    it('GET /curricula - Should list active curricula', async () => {
        const res = await request(app.getHttpServer()).get('/curricula').expect(200);
    ...
    it('GET /curricula/:id/hierarchy - Should return optimized tree', async () => {
        ...
        expect(res.body.stages[0].sequence).toBeLessThan(res.body.stages[1].sequence);
    });
    ...
    it('Search - MODE A: Broad Search (No Grade Filter)', async () => {
        ...
        const found = res.body.find((listing: any) =>
