...
describe('Package Reschedule Integration Tests', () => {
    ...
    beforeEach(async () => {
        mockPrisma = {
            ...
            teacherAvailabilityException: { findMany: jest.fn().mockResolvedValue([]) },
        ...
        jest.spyOn(bookingService, 'validateSlotAvailability').mockResolvedValue(true);
    });
        ...
        it('should reject reschedule within window hours', async () => {
            ...
            await expect(
                bookingService.reschedulePackageSession(
                    'student-1', 'STUDENT', 'booking-1',
                    newTime, new Date(newTime.getTime() + 60 * 60 * 1000)
                )
    ...
    describe('Status Enforcement', () => {
        ...
        const forbiddenStatuses = ['PENDING_CONFIRMATION', 'COMPLETED', 'CANCELLED', 'DISPUTED'];
    ...
    describe('Availability Check', () => {
        ...
        it('should reject when teacher is not available', async () => {
            mockPrisma.booking.findUnique.mockResolvedValue(mockPackageBooking);
            jest.spyOn(bookingService, 'validateSlotAvailability').mockResolvedValue(false);
    ...
    describe('Teacher Reschedule Request', () => {
        ...
        it('should decline reschedule request without changing booking', async () => {
            ...
            const result = await bookingService.declineRescheduleRequest('student-1', 'request-1');
