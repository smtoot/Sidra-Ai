...
const envPath = path.resolve(process.cwd(), 'packages/database/.env');
    ...
    if (fs.existsSync(envPath)) {
        ...
        envConfig.split('\n').forEach(line => {
            if (line && line.indexOf('=') !== -1 && !line.startsWith('#')) {
                ...
                const value = val.join('=').trim().replace(/^["']|["']$/g, ''); // Remove quotes
...
async function lockFundsForBooking(walletId, amount, userId, bookingId) {
...
async function main() {
    console.log('🚀 Starting Wallet Concurrency Test (JS/CJS)...');
